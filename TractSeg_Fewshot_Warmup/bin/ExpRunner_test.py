...
def main():
    ...
    if Config.TEST:
        ...
        Config.WEIGHTS_PATH = '/data7/wanliu/TractSeg_Run/hcp_exp2/my_custom_experiment_x22/best_weights_ep152.npz'
        save_path = '/data7/wanliu/TractSeg_Run/hcp_exp2/my_custom_experiment_x22'
        inference = True
        cal_metric = True
        ...
        if cal_metric:
            ...
            if tract_num == 12:
                bundles = ['CST_left', 'CST_right', 'FPT_left', 'FPT_right', 'ILF_left', 'ILF_right', 'OR_left',
            ...
            for subject_index in range(len(all_subjects)):
                ...
                for tract_index in range(label.shape[-1]):
                    Dice_all[subject_index, tract_index] = compute_dice_score(seg[:, :, :, tract_index],
                                                                              label[:, :, :, tract_index])
                    IOU_all[subject_index, tract_index] = compute_iou_score(seg[:, :, :, tract_index],
                                                                            label[:, :, :, tract_index])
                    RVD_all[subject_index, tract_index] = compute_rvd_score(seg[:, :, :, tract_index],
                                                                            label[:, :, :, tract_index])

                with open(join(save_path, "test_dice***_" + str(test_epoch) + ".txt"), 'a') as f:
                    f.write('Dice of subject {} is \n {} \n'.format(subject, Dice_all[subject_index, :]))
                with open(join(save_path, "test_iou***_" + str(test_epoch) + ".txt"), 'a') as f:
                    f.write('IOU of subject {} is \n {} \n'.format(subject, IOU_all[subject_index, :]))
                with open(join(save_path, "test_rvd***_" + str(test_epoch) + ".txt"), 'a') as f:
            ...
            with open(join(save_path, "test_dic***_" + str(test_epoch) + ".txt"), 'a') as f:
