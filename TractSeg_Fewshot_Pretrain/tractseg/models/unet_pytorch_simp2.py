...
class UNet_Pytorch_Simp2(torch.nn.Module):
    def __init__(self, n_input_channels=3, n_classes=7, n_filt=64, batchnorm=False, dropout=False,upsample="bilinear"):
        ...
        self.conv_5_seg = nn.Conv2d(n_filt, n_classes, kernel_size=1, stride=1, padding=0, bias=True)



        self.contr_1_1_add = conv2d(n_classes, n_filt, batchnorm=batchnorm)
        self.contr_1_2_add = conv2d(n_filt, n_filt, batchnorm=batchnorm)
        self.pool_1_add = nn.MaxPool2d((2, 2))

        self.contr_2_1_add = conv2d(n_filt, n_filt * 2, batchnorm=batchnorm)
        self.contr_2_2_add = conv2d(n_filt * 2, n_filt * 2, batchnorm=batchnorm)
        self.pool_2_add = nn.MaxPool2d((2, 2))

        self.dropout_add = nn.Dropout(p=0.4)

        self.encode_1_add = conv2d(n_filt * 2, n_filt * 4, batchnorm=batchnorm)
        self.encode_2_add = conv2d(n_filt * 4, n_filt * 4, batchnorm=batchnorm)
        self.deconv_1_add = deconv2d(n_filt * 4, n_filt * 4, kernel_size=2, stride=2)

        self.expand_1_1_add = conv2d(n_filt * 4 + n_filt * 2, n_filt * 2, stride=1, batchnorm=batchnorm)
        self.expand_1_2_add = conv2d(n_filt * 2, n_filt * 2, stride=1, batchnorm=batchnorm)
        self.deconv_2_add = deconv2d(n_filt * 2, n_filt * 2, kernel_size=2, stride=2)

        self.expand_2_1_add = conv2d(n_filt * 2 + n_filt * 1, n_filt * 1, stride=1, batchnorm=batchnorm)
        self.expand_2_2_add = conv2d(n_filt * 1, n_filt * 1, stride=1, batchnorm=batchnorm)
        self.conv_5_add = nn.Conv2d(n_filt, n_classes, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, input):
        ...
        expand_4_2 = self.expand_4_2_seg(expand_4_1)
        conv_5_seg = self.conv_5_seg(expand_4_2)



        contr_1_1 = self.contr_1_1_add(conv_5_seg)
        contr_1_2 = self.contr_1_2_add(contr_1_1)
        pool_1 = self.pool_1_add(contr_1_2)

        contr_2_1 = self.contr_2_1_add(pool_1)
        contr_2_2 = self.contr_2_2_add(contr_2_1)
        pool_2 = self.pool_2_add(contr_2_2)

        if self.use_dropout:
            pool_2 = self.dropout_add(pool_2)

        encode_1 = self.encode_1_add(pool_2)
        encode_2 = self.encode_2_add(encode_1)
        deconv_1 = self.deconv_1_add(encode_2)

        concat1 = torch.cat([deconv_1, contr_2_2], 1)
        expand_1_1 = self.expand_1_1_add(concat1)
        expand_1_2 = self.expand_1_2_add(expand_1_1)
        deconv_2 = self.deconv_2_add(expand_1_2)

        concat2 = torch.cat([deconv_2, contr_1_2], 1)
        expand_2_1 = self.expand_2_1_add(concat2)
        expand_2_2 = self.expand_2_2_add(expand_2_1)
        conv_5_seg_add = self.conv_5_add(expand_2_2)

        return conv_5_seg_add