...
class UNet_Pytorch_DeepSup_Fpn(torch.nn.Module):
    def __init__(self, n_input_channels=3, n_classes=7, n_filt=64, batchnorm=False, dropout=False, upsample="bilinear"):
        ...
        ### New added feature pyramid network to refine the feature pyramid of encoding path
        # upsampling 2*
        self.upconv_4 = deconv2d(n_filt * 16, n_filt*8, kernel_size=2, stride=2)
        self.upconv_3 = deconv2d(n_filt * 8, n_filt*4, kernel_size=2, stride=2)
        self.upconv_2 = deconv2d(n_filt * 4, n_filt*2, kernel_size=2, stride=2)
        self.upconv_1 = deconv2d(n_filt * 2, n_filt*1, kernel_size=2, stride=2)

        # 3*3 conv feature second refinement stage
        self.refine_4 = conv2d(n_filt*8,n_filt*8, stride=1)
        self.refine_3 = conv2d(n_filt*4,n_filt*4, stride=1)
        self.refine_2 = conv2d(n_filt*2,n_filt*2, stride=1)
        self.refine_1 = conv2d(n_filt*1,n_filt*1, stride=1)
    ...
    def forward(self, inpt):
        ...
        ### Insert FPN to refine the feature pyramid in the encoding path
        upconv_4 = self.upconv_4(encode_2)
        upconv_4_add = upconv_4 + contr_4_2
        upconv_3 = self.upconv_3(upconv_4_add)
        upconv_3_add = upconv_3 + contr_3_2
        upconv_2 = self.upconv_2(upconv_3_add)
        upconv_2_add = upconv_2 + contr_2_2
        upconv_1 = self.upconv_1(upconv_2_add)
        upconv_1_add = upconv_1 + contr_1_2

        # without 3*3
        # contr_4_2 = upconv_4_add
        # contr_3_2 = upconv_3_add
        # contr_2_2 = upconv_2_add
        # contr_1_2 = upconv_1_add

        # 3*3 convolution for second refinement stage
        contr_4_2 = self.refine_4(upconv_4_add)
        contr_3_2 = self.refine_3(upconv_3_add)
        contr_2_2 = self.refine_2(upconv_2_add)
        contr_1_2 = self.refine_1(upconv_1_add)
