...
def main():
    ...
    if Config.TEST:
        Config.WEIGHTS_PATH = '/data6/wanliu/TractSeg_Run/hcp_exp2/my_custom_experiment_x17/best_weights_ep120.npz'
        Test_peak_dir = '/data4/wanliu/HCP_Wasser/HCP_for_training_COPY'
        save_path = '/data6/wanliu/TractSeg_Run/hcp_exp2/my_custom_experiment_x17'
        inference = True
        cal_metric = True
        ...
        if cal_metric:
            ...
            IOU_all = np.zeros([len(all_subjects), 72])
            RVD_all = np.zeros([len(all_subjects), 72])
            ...
            for subject_index in range(len(all_subjects)):
                ...
                with open(join(save_path, "test_iou_"+str(test_epoch)+".txt"), 'a') as f:
                    f.write('IOU of subject {} is \n {} \n'.format(subject, IOU_all[subject_index, :]))
                with open(join(save_path, "test_rvd_"+str(test_epoch)+".txt"), 'a') as f:
